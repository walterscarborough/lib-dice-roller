...
pub fn convert_js_array_to_protobuf(js_array_data: &JsValue) -> Vec<u8> {
    let input_iterator = js_sys::try_iter(js_array_data).expect("should be able to iterate through input data").unwrap();
    ...
    for x in input_iterator {
        ...
        if x.as_f64().is_some() {
            let x = x.as_f64().expect("should be able to convert from JS number to Rust number") as u8;
            rust_data.push(x);
        }
    }
